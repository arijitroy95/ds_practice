...
the same for a train but we can have arrival time of one train equal to departure time of the other.
...
def platform_count(arrivals, departures, n):
    arrivals.sort()
    departures.sort()
    req_plats = 1
    result = 1
    ...
    while i < n and j < n:
        if arrivals[i] <= departures[j]:
            ...
            req_plats -= 1
            j += 1
        result = max(result, req_plats)

    return result


arrs = [900, 940, 950, 1100, 1500, 1800]
deps = [910, 1200, 1120, 1130, 1900, 2000]
print(platform_count(arrs, deps, 6))
